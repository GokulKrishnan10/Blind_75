...
ascending order by starting and intervals still does not have any overlapping intervals (merge overlapping intervals if necessary).
...
class Solution {
    public int[][] insert(int[][] intervals, int[] newInterval) {
        ...
        for(int[] arr:list){
            if(l.isEmpty() || l.getLast()[1]<arr[0]){
                l.add(arr);
            }
            else{
                l.getLast()[1]=Math.max(arr[1],l.getLast()[1]);
            }
        }
