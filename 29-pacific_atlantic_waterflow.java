...
class Solution {
    int m,n;
    public boolean isValid(int[][] A,int[] p,int r,int c,boolean[][] visited){
        return r>=0 && c>=0 && r<m && c<n && !visited[r][c] && A[p[0]][p[1]]<=A[r][c];
    }
    public void bfs(int[][] A,int r,int c,boolean[][] visited){
        ...
        while(!q.isEmpty()){
            ...
            for(int i=0;i<4;i++){
                int pX=pair[0]+row[i];
                int pY=pair[1]+col[i];
                if(isValid(A,pair,pX,pY,visited)){
                    q.add(new int[]{pX,pY});
                    visited[pX][pY]=true;
                }
            }
