...
class Solution {
    ...
    public void dfs(int index,int[] candidates,int target,LinkedList<Integer> l,int n){
        if(index>=n)return;
        if(target==0){
        ...
        if(candidates[index]<=target){
            ...
            dfs(index,candidates,target-candidates[index],l,n);
            l.removeLast();
        }
        dfs(index+1,candidates,target,l,n);
    }
    public List<List<Integer>> combinationSum(int[] candidates, int target) {
        int n=candidates.length;
        dfs(0,candidates,target,new LinkedList(),candidates.length);
        return list;
    }
}